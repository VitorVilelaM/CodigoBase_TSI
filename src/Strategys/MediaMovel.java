...
public class MediaMovel implements Strategy {
    ...
    private double lDay, lYesterday; // Media movel longa / Preciso do valor atual e a do dia anterior / Media Pricinpal
    private double cDay, cYesterday; // Media movel curta  / Preciso do valor atual e a do dia anterior
    ...
    public void Update(MarketData update) {
        ...
        if (this.num_days > this.mmLonga.length) {
            if ((this.lDay > this.cDay) && (this.lYesterday < this.cYesterday) && (this.state != State.SHORT)) {
                Venda(update);
            } else if ((this.lDay < this.cDay) && (this.lYesterday > this.cYesterday) && (this.state != State.LONG)) {
                Compra(update);
            }
        }

        this.cYesterday = this.cDay;
        this.lYesterday = this.lDay;
        this.num_days += 1;
    }
